                    ...
                    <div class="flex flex-col items-center relative">
                        ...
                        <div class="w-full">
                            <div class="my-2 p-1 bg-white flex border border-gray-200 rounded">
                                ...
                                placeholder="Buscar Docentes... " class="p-1 px-2 appearance-none outline-none w-full text-gray-800">

                                <div class="text-gray-300 w-8 py-1 pl-2 pr-1 border-l flex items-center border-gray-200">
                                    <button wire:click="restablecer()" class="cursor-pointer w-6 h-6 text-gray-600 outline-none focus:outline-none">
                        ...
                        <div class=" shadow bg-white top-100 z-40 w-full lef-0 rounded max-h-select overflow-y-auto svelte-5uyqqj">
                            <div class="flex flex-col w-full">
                                ...
                                <div class="cursor-pointer w-full border-gray-100 border-b hover:bg-teal-100">
                                    <div wire:click="asignarUsuario('{{ $usuario->name }} {{ $usuario->lastname }}',{{ $usuario->id }})" class="flex w-full items-center p-2 pl-2 border-transparent border-l-2 relative hover:border-teal-100">
                                        <div class="w-6 flex flex-col items-center">
                                            @if ($usuario->profile_photo_path == null)
                                            <div class="flex relative w-10 h-10 bg-orange-500 justify-center items-center m-1 mr-2 w-4 h-4 mt-1 rounded-full "><img class="rounded-full"  src="{{ $usuario->image_user }}" alt="{{ $usuario->name }} "> </div>
                                            @else
                                            <div class="flex relative w-10 h-10 bg-orange-500 justify-center items-center m-1 mr-2 w-4 h-4 mt-1 rounded-full "><img class="rounded-full" alt="{{ $usuario->name }}" src="{{ asset('storage/' . $usuario->profile_photo_path) }}"> </div>
                                            @endif
                                        </div>
                                        <div class="w-full items-center flex">
                                            <div class="mx-2 -mt-1  ">{{ $usuario->name }} {{ $usuario->lastname }}
                                                <div class="text-xs truncate w-full normal-case font-normal -mt-1 text-gray-500">{{ $usuario->r_area->nombre_area }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <select wire:model="informe_realizado"  id="input4"
                ...
                <option value="{{ $informe->id }}">{{ $informe->fecha_inicio_realizadas }}/{{ $informe->fecha_fin_planificadas }} enviado: {{$informe->updated_at}}</option>
                @endforeach
            </select>
        ...
        <div  class="w-full md:w-1/2 px-3">
            ...
            <select id="input4"
                wire:model="informe_planificado" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                <option value="" selected>Seleccione... </option>
                @foreach ($this->informes as $informe)
                <option value="{{ $informe->id }}" >{{ $informe->fecha_inicio_realizadas }}/{{ $informe->fecha_fin_planificadas }} enviado: {{$informe->updated_at}}</option>
                @endforeach
...
<div class="p-8 bg-white">
    ...
    <div class="flex items-center space-x-2 mt-20">
      ...
      <img
        class="w-10 h-10 object-cover object-center rounded-full"
        alt="{{ $user->name }}" src="{{ asset('storage/' . $user->profile_photo_path) }}"
      />
      @endif
      <div>
        <!--Author name-->
